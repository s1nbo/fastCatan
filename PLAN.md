...
- [x] ✅ **De-catanatronization COMPLETE — self-contained ceiling ~17% (2026-06-08).** All three stages built + measured on the native AB-d2 dev ladder (200 g, 512 sims, seed 12345): **(1) learned leaf value** (`--leaf-eval net`) — best is a final-outcome head (`--value-target vp_margin`) at **20.0% [15.1–26.1]** (leaf-only, symbolic opponent); directly distilling the two-scale `ab_value` squash (`--value-target ab_two_scale`) was *worse*, 16.5% — mimicking the info-capped heuristic doesn't transfer. **(2) learned in-tree opponent** (net masked-argmax replaces `ab_decide`, `--model-opp net`) → FULLY SELF-CONTAINED: **18.0% [13.3–23.9]** (Cell G, 160k clone) / 16.5% (Cell H, 640k/4× data). **(3) search-improved value targets** — distill the hybrid's 512-sim MCTS root value (`stage3_gen.py` + `il_pretrain --value-target search_value --init-from`): **17.5% [12.9–23.4]** (Cell I) — a clean **NULL** (value fit well, mse 0.0136 / top-1 0.904, wins flat). **Ledger: hybrid 29.5 (native) / learned-leaf 20.0 / self-contained ~17 / parity 25.** The ~12-pt cost of self-containment is an **information cap**, not compute/data/target: the POV obs cannot see the hidden enemy state `ab_value` reads (the `max` over opponents' unplayed devs/hands). Confirmed irreducible — capacity, epochs, 4× data, sims, and search-improved targets all falsified. The hybrid (32.5% bridge GATE PASS) keeps that information edge and is the reference; the self-contained agent is information-bounded. Full ladder + mechanism: `EVAL/AB/README.md`, memory `learned-judge-distillation`.
- **Thesis gate (final tier): self-contained agent, ≥1000 bridge games vs Alpha-Beta d2, Wilson CI lower bound > 0.25.** Dev tier: native AB-d2 ladder (≥200 games, ≥512 sims), CI-low > 0.25 to promote. **Outcome (2026-06-08): the HYBRID clears the gate (32.5% bridge), but it is not self-contained; the self-contained agent caps at ~17% < 25%, so the gate is NOT met self-contained. The characterized information cap (above) is the thesis result, with the hybrid gate-pass as the positive reference.**
